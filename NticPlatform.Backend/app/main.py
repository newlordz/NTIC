...
try:
    ...
    def export_report(
        competition_id: str = "",
        report_type: str = "Squad Roster Export",
        format: str = "csv",
        school: str = "",
        actor: dict = Depends(require_auth)
    ):
        """
        Server-generated report export stamped with cycle name, date, and scope.
        Returns a downloadable CSV or TXT file with strict cycle scoping.
        """
        import io
        import csv
        ...
        if competition_id:
            cur.execute("SELECT title FROM competitions WHERE id = %s", (competition_id,))
            comp_row = cur.fetchone()
            if comp_row and comp_row[0]:
                cycle_name = comp_row[0].strip()
            else:
                cycle_name = f"Cycle ({competition_id})"

        actor_role = actor.get("role", "")
        actor_org = (actor.get("organization") or "").strip()
        effective_school = school.strip()
        if actor_role == ROLE_SCHOOL_ADMIN and actor_org:
            effective_school = actor_org

        query = "SELECT id, name, track, lead, members, status, school_name, competition_id, mentor, roster_list FROM teams WHERE 1=1"
        params = []
        if competition_id:
            query += " AND competition_id = %s"
            params.append(competition_id)
        if effective_school:
            query += " AND (school_name = %s OR school_name ILIKE %s)"
            params.extend([effective_school, f"%{effective_school}%"])

        query += " ORDER BY school_name ASC, name ASC"
        ...
        iso_stamp = now_utc.strftime("%Y-%m-%d %H:%M:%S UTC")

        clean_cycle = re.sub(r'[^a-zA-Z0-9_-]+', '_', cycle_name).strip('_').lower() or "all_cycles"
        clean_type = re.sub(r'[^a-zA-Z0-9_-]+', '_', report_type).strip('_').lower() or "report"
        fmt_lower = format.strip().lower()

        is_csv = fmt_lower in ("csv", "excel")
        ext = "csv" if is_csv else "txt"
        filename = f"ntic_report_{clean_cycle}_{clean_type}_{date_stamp}.{ext}"

        output = io.StringIO()
        if is_csv:
            writer = csv.writer(output)
            output.write(f"# NTIC NATIONAL TECH AND INNOVATION CHAMPIONSHIP\n")
            output.write(f"# Report Title: {report_type}\n")
            output.write(f"# Cycle Scope: {cycle_name}\n")
            output.write(f"# Export Date: {iso_stamp}\n")
            output.write(f"# Filtered Institution: {effective_school or 'All Institutions'}\n")
            output.write(f"# Generated By: {actor.get('fullName') or actor.get('email', 'User')} ({actor_role})\n")
            ...
            for r in rows:
                ...
                if isinstance(roster, str):
                    try:
                        ...
                        roster = []
                roster_str = ", ".join([str(m) for m in (roster or [])]) if isinstance(roster, list) else ""
            ...
            output.write(f"==========================================================\n")
            output.write(f"         NTIC NATIONAL COMPETITION PLATFORM\n")
            output.write(f"        OFFICIAL INSTITUTIONAL REPORT EXPORT\n")
            output.write(f"==========================================================\n\n")
            output.write(f"Report Title    : {report_type}\n")
            output.write(f"Cycle Scope     : {cycle_name}\n")
            output.write(f"Generated Date  : {iso_stamp}\n")
            output.write(f"Institution     : {effective_school or 'All Institutions'}\n")
            output.write(f"Compiled By     : {actor.get('fullName') or actor.get('email', 'User')} ({actor_role})\n")
            output.write(f"Total Records   : {len(rows)}\n\n")
            output.write(f"--- SQUAD ROSTER DETAILS ---\n")
            if rows:
                for i, r in enumerate(rows):
                    ...
                    if isinstance(roster, str):
                        try:
                            ...
                            roster = []
                    roster_str = ", ".join([str(m) for m in (roster or [])]) if isinstance(roster, list) else "N/A"
                    output.write(f"{i + 1}. Squad Name : {r[1]}\n")
                    output.write(f"   Squad ID   : {r[0]}\n")
                    output.write(f"   Track      : {r[2]}\n")
                    output.write(f"   School     : {r[6]}\n")
                    output.write(f"   Lead       : {r[3]}\n")
                    output.write(f"   Members    : {roster_str}\n")
                    output.write(f"   Cycle      : {cycle_name}\n\n")
            else:
                output.write(f"No squads found for {cycle_name}.\n\n")
            output.write(f"==========================================================\n")
            output.write(f"End of Official Report - Ghana Data Protection Act Compliant\n")

        media_type = "text/csv; charset=utf-8" if is_csv else "text/plain; charset=utf-8"
        return Response(
